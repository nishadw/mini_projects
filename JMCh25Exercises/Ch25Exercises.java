...
public class Ch25Exercises
{
    ...
    {
        int len = (int) completeTree[0];
        for (int i = 1; i <= len; i++) {
            if (2 * i > len) {
                completeTree[i] = null;
                completeTree[0] = (Integer) completeTree[0] - 1;
            }
        }
    }
    /**
    ...
    public Object[] toArray( TreeNode root, int numNodes )
    {
        Object[] objArr = new Object[numNodes + 1];
        objArr[0] = numNodes;
        toArray(root, objArr, 1);
        return objArr;
    }
    ...
    {
        ArrayList<Integer> leaves = new ArrayList<Integer>(items.length);
    ...
    {
        PriorityQueue<Message> finalQueue = new PriorityQueue<Message>();
