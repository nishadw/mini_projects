 ...
 * 
 * @author Period - 6
 * @author Assignment - getAssignment()
 * 
 * @author Sources - mi duelo
 */
public class Actor implements Comparable<Actor>
{
    ...
    public void readMail()
    {
        ...
        while (!mailbox.isEmpty()) {
            ...
            if (msg.getText().contains("need")) {
                String str = msg.getText().replace("need ", "");

                if (countPossessions(str) > 1) {
                    reply(msg.getSender(), "have " + str);
                }
            }

            if (msg.getText().contains("have")) {
                String str = msg.getText().replace("have ", "");

                if (countPossessions(str) == 0) {
                    reply(msg.getSender(), "ship " + str);
                }
            }

            if (msg.getText().contains("ship")) {
                String str = msg.getText().replace("ship ", "");

                if (countPossessions(str) > 1) {
                    if (msg.getSender().receiveItem(msg.getSender(), str)) {
                        myPossessions.remove(str);
                    }
                }
            }
        }
