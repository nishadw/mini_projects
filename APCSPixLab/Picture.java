...
public class Picture extends SimplePicture
{
    ...
    {
        Pixel[][] pixelBoard = this.getPixels2D();
        Pixel pixel = null;
        for (int r = 0; r < pixelBoard.length; r++) {
            for (int c = 0; c < pixelBoard[0].length; c++) {
                pixel = pixelBoard[r][c];
    ...
    public void grayscale()
    {
        ...
        for (int r = 0; r < pixelBoard.length; r++) {
            for (int c = 0; c < pixelBoard[0].length; c++) {
                ...
                int grey = (pixel.getBlue() + pixel.getRed() + pixel.getGreen()) / 3;
...
//        }
    ...
    public void edgeDetection2( int edgeDist )
    {
        ...
        Pixel[][] pixel2 = copy.getPixels2D();
        Pixel top = null;
        Pixel bottom = null;
        Color color = null;

        for (int r = 0; r < pixel2.length - 1; r++) {
            for (int c = 0; c < pixel2[0].length; c++) {
                top = pixel2[r][c];
                bottom = pixel2[r + 1][c];
                color = bottom.getColor();

                if (edgeDist < top.colorDistance(color)) {
