...
public class MyTreeSet<E>
{
    ...
    private int width( TreeNode<E> root )
    {
        ...
        int width = 0;
        ArrayList<TreeNode<E>> list = new ArrayList<TreeNode<E>>(0);
        list.add(root);
    ...
    private TreeNode<E> findPtr( TreeNode<E> root, Comparable target )
    {
        ...
        //int val = root.getValue().compareTo(target); <-- doesn't work for some reason?
        int val = target.compareTo(root.getValue());
    ...
    private int[] nodeCounts( TreeNode<E> root )
    {
        ...
        if (root == null) {
            ...
            int right = countNodesHelper(root.getRight());
            int left = countNodesHelper(root.getLeft());

            result[0] = 1 + right + left;
            result[1] = right - left;
            return result;
        }
    }
    ...
    // nodes of its subtree (inclusive of the value of root).
    ...
    private TreeNode<E> trim( TreeNode<E> root )
    {
        ...
        if (root == null || height(root) == 1) {
            ...
            TreeNode<E> right = trim(root.getRight());
            TreeNode<E> left = trim(root.getLeft());
    ...
    private double minMaxAverage( TreeNode<Integer> root )
    {
        ...
        double num = (right.getValue() + left.getValue()) / 2.0;
