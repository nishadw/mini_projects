 ...
 * examined before every move. One counts the number of free new choices -
 * entrances - every one of them has, and then moves to the square with the
 * lowest number of new choices. <br />
 * 
 * @author Nishad Wajge
 * @version 11/19
 * @author Period: 6
 * @author Assignment: KnightCritterTour
 * @author Sources: es mio.
 */
public class KnightTourCritter
    extends KnightCritter
{
    ...
    public Location getBestMove(ArrayList<Location> locs)
    {
        ...
        for (Location l : locs)
        {
            ...
            if (accessGrid[row][col] < x && accessGrid[row][col] > 0)
            {
                bestMove = new Location(row, col);
                x = accessGrid[row][col];
            }
    ...
    public Location selectMoveLocation(ArrayList<Location> locs)
    {
        ...
        Location optimal = getBestMove(locs);
